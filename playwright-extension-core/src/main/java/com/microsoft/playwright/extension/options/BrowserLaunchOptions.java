...
public class BrowserLaunchOptions {
    ...
    public BrowserType.LaunchOptions toOptions() {
        ...
        map.from(this.getArgs()).when(args -> ((args) != null && !(args).isEmpty())).to(options::setArgs);
        ...
        map.from(this.getEnv()).when(env -> ((env) != null && !(env).isEmpty())).to(options::setEnv);
        map.from(this.getExecutablePath()).whenNonNull().to(options::setExecutablePath);
        map.from(this.getFirefoxUserPrefs()).when(firefoxUserPrefs -> ((firefoxUserPrefs) != null && !(firefoxUserPrefs).isEmpty())).to(options::setFirefoxUserPrefs);
        ...
        map.from(this.getIgnoreDefaultArgs()).when(ignoreDefaultArgs -> ((ignoreDefaultArgs) != null && !(ignoreDefaultArgs).isEmpty())).to(options::setIgnoreDefaultArgs);
